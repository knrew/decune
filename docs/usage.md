...
固定 gateway や subnet を service の environment に埋め込む構成では、`[[compose.clone_isolation.endpoints]]` で対象 service・環境変数・値 template を宣言してください。`${decune.network.<network-key>.gateway}` と `.subnet` は relocation 後の値へ展開されます。実際に relocate した network の旧 gateway / subnet 基底 address が宣言なしで environment に残る場合、`decune up` は `compose_clone_isolation_endpoint_unsafe` で起動前に停止します。v1 の stale 検出対象は environment のみで、`extra_hosts`、command、config file 内の address は利用者が追随させる必要があります。
