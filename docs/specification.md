...
- 自 project の既存 network と新 plan の subnet が異なる場合、接続 container がなければ network を削除し、Compose に再作成させる。接続 container がある場合は `compose_clone_isolation_invalid` で停止し、project を停止して `decune rebuild` するよう案内する。
