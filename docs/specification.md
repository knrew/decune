...
`endpoints.value` では `${decune.network.<compose-network-key>.gateway}` と `${decune.network.<compose-network-key>.subnet}` の 2 形式を clone isolation 専用 placeholder として予約する。これは一般の decune config 変数展開とは別に扱う。endpoint rewrite preflight は service と Compose network key の存在、および参照先 network が固定 IPv4 subnet relocation の対象であることを検証し、placeholder を planned gateway または CIDR 表記の planned subnet へ文字列置換する。未知または未終端の decune placeholder、存在しない service / network、relocation 対象でない network への参照は `compose_clone_isolation_invalid` で error にする。
...
network が実際に別 subnet へ relocate された場合、preflight は canonical Compose model の `services.*.environment` の string value を走査する。元 subnet の基底 IPv4 address、または元 gateway が、前後を数字・dot としない token 境界付きで残っており、同じ service と環境変数名の endpoint 宣言に覆われていなければ、`compose_clone_isolation_endpoint_unsafe` で `docker compose up` 前に error にする。`10.99.0.1` は `10.99.0.100` や `110.99.0.1` に一致しない。planned subnet が requested subnet と同じ場合は stale 検出を行わない。
