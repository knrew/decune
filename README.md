...
- image-based / Dockerfile-based / Docker Compose-based の Dev Container 構成の検出と起動
- `decune up` だけで build、起動、remote user のシェル接続までを実行
- `rebuild` / `down` / `status` / `ports` / `remove` / `clean` による明示的なライフサイクル管理と状態確認
- Dev Container Features、dotfiles、Git/GitHub 認証情報転送、Linux UID/GID 同期の適用
- decune のポートフォワーディングと Docker published port の管理・一覧表示
- global と project の 2 層の decune TOML 設定を `devcontainer.json` へ重ね合わせ
- 同じ Docker Compose-based リポジトリの複数 clone の同時利用(opt-in の衝突分離)

## 対象範囲

Linux / macOS ホストと Docker CLI / Docker Compose v2 プラグインを対象にします(旧 `docker-compose` v1 standalone binary は対象外)。Docker Compose-based 構成では、`service` で指定した Compose サービスを主対象にシェル接続とライフサイクル管理を適用し、Compose file の解釈と実行時設定は Docker Compose に委譲します。
...
- 認証情報転送は、ホストの Git 認証情報、SSH agent、GitHub token file への到達性をコンテナ内プロセスに与え得ます。信頼していないリポジトリでは、無効化するか read-only に制限してください。
...
- [docs/configuration.md](docs/configuration.md): decune TOML 設定の使い方と挙動説明のガイド
...
- [docs/specification.md](docs/specification.md): 公開挙動、CLI 契約、設定スキーマ、セキュリティ境界、診断コード定義の正本
